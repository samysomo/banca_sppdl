...
const formSchema = z.object({
  description: z.string().min(4, "Transfer description is too short"),
  amount: z.string().min(1, "Amount is too short"),
  from_account_id: z.string().min(1, "Please select a valid bank account"),
  to_account_id: z.string().min(1, "Please select a valid bank account id"),
});
...
const PaymentTransferForm = ({ accounts }: PaymentTransferFormProps) => {
  ...
  const submit = async (data: z.infer<typeof formSchema>) => {
    setIsLoading(true);
    try {
      console.log("Tried to create transfer")
      ...
      if (transactionData.amount <= 0) return "La cantidad no puede ser menor o igual a 0"
      console.log(transactionData)
        const makeTransfer = async () => {
          if (!token || !transactionData) return "Campos incompletos"
          try {
              const request = await createTransfer({token, transactionData})
              if(request){
                  alert(request)
              } else {
                  alert("No jalo")
